...
- [ ] On POST failure, the banner reappears with an inline error and focus returns to the dismiss button.
- [ ] On POST success, focus has already moved to a stable target before the banner hides.
- [ ] Manual keyboard test: tab to dismiss, press Enter, confirm focus lands on a sensible target (not body).
- [ ] Manual offline test: dismiss with network disabled, confirm banner reappears with error message.
- [ ] No console errors thrown when banner element is removed mid-animation.
