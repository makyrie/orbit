...
- [ ] `Orbit_Messaging_Copy::sms_status_clause()` exists and returns the SMS clause only when `! sms_enabled()`.
- [ ] All four sites compose copy via the helper, not via inline strings.
- [ ] Flipping `sms_enabled()` in test removes the SMS clause from all four sites.
- [ ] `compliance_disclosure_text()` output differs as expected when the flag flips, and the snapshot-equality test (todo 124) covers both variants.
- [ ] Grep for "SMS coming soon" and the heredoc phrase returns no untreated hits.
